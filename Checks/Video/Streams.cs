...
namespace Auditor.Checks.Video
{
	...
	public class Streams : VideoCheck
	{
		public override string Name => "Video Streams";
		...
		public override CheckResult Test(VideoTarget target)
		{
			...
			var streams = target.Properties.AudioStreams.Count + target.Properties.VideoStreams.Count;
			...
			if (this.Min != null && streams < this.Min)
			{
				...
				{
					Success = false,
					Messages = new[] { $"Streams count ({streams}) is below minimum of {this.Min}" }
				};
			}
			...
			if (this.Max != null && streams > this.Max)
			{
				...
				{
					Success = false,
					Messages = new[] { $"Streams count ({streams}) is above maximum of {this.Max}" }
				};
			}
			...
			if (this.Exactly != null && streams != this.Exactly)
			{
				...
				{
					Success = false,
					Messages = new[] { $"Streams count ({streams}) does not match {this.Exactly}" }
				};
			}
